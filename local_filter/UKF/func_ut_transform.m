...
for i = 1 : 2 * n + 1
    if i == 1
        P_tfr = W_c(i) * (Y(:, i) - m_tfr) * (Y(:, i) - m_tfr)';
    else
        P_tfr = P_tfr + W_c(i) * (Y(:, i) - m_tfr) * (Y(:, i) - m_tfr)';
    end
end
% calculate `P_cov_tfr`
P_cov_tfr = [];
for i = 1 : 2 * n + 1
    if i == 1
        P_cov_tfr = W_c(i) * (X(:, i) - m) * (Y(:, i) - m_tfr)';
    else
        P_cov_tfr = P_cov_tfr + W_c(i) * (X(:, i) - m) * (Y(:, i) - m_tfr)';
    end
end
